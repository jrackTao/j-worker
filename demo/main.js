 ...
 */
var createWorkerAsync = (carrier, autoDestroy = true) => {
...
/**
 * 获取线程数最优数量
 * @param {Number} defaultCount 初始线程数
 * @param {Number} defaultMax 最大计算量
 */
const getOptimalCountOfThreads = async (defaultCount = 2, defaultMax = 1e6) => {
	...
	while(count++) {
		...
		console.log(`cpu 数量${count}，用时：${newUseTime}ms `)
...
getOptimalCountOfThreads(4, 1e6).then(count => {
	console.log(`最优解是:${count}`)
})
